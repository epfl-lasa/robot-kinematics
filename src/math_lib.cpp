...
unsigned int matsvdinv_self( int m, int n, double **out_mat)
{
	double **mat;
	mat = smatrix(m, n);
	matsvdinv( out_mat, m, n, mat);
	matcp(m, n, mat, out_mat );
...
void matzero( int m, int n, double **out_vec )
{
    int i,j;
...
void matmul_stranspose( int mn, double **lmat, double **rmatt, double **out_mat)
{
	int i,j,k;
	...
	for(j=0; j<mn; j++)
	{
	    out_mat[i][j] = 0.;
	    for(k=0; k<mn; k++)
			out_mat[i][j] += lmat[i][k]*rmatt[j][k];
	}	
}
...
void matadd( int m, int n, double **mat, double **out_mat )
{
    int i, j;
...
void matsub_c1( int m, double *lvec, double *rvec, double *out_vec )
{
    int i;
...
void matsub( int m, int n, double **lmat, double **outmat)
{
	int i,j;
	for(i=0; i<m; i++)
	for(j=0; j<m; j++)
		outmat[i][j] = lmat[i][j] - outmat[i][j];
...
void matI( int m, double **out_mat)
{
    int i,j;
...
void matcp(int m, int n, double **in_mat, double **out_mat)
{
	int i,j;
    for(i=0; i<m; i++)
	for(j=0; j<n; j++)
...
void matinv_c3( double mat[][3], double out_mat[][3] )
{
	...
	matsvdinv(mat1, 3, 3, mat2);

	for( i=0; i<3; i++ ) 
		for( j=0; j<3; j++) 
			out_mat[i][j] = mat2[i][j];

}

void matinv_c3_( double mat[][3], double out_mat[][3] )
{
    int i, j, k;
    double a1, a2;
    int m = 3;
...
int matpivot_c15( int nm, double mat[][15] )
{
    ...
    amax = -1.;
    for (i=nm; i<m; i++)
	if ( (temp = dabs_m(mat[i][nm])) > amax && temp != 0.0)
	{
	    amax = temp;
	    k = i;
	}
